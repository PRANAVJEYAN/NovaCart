...
import { login, clearRedirectAfterLogin } from '../store/slices/authSlice'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.meta.requestStatus === 'fulfilled') {
      ...
      const destination = result.payload.user.role === 'admin' ? '/admin/dashboard' : returnTo
  ...
  return (
    <div className="mx-auto max-w-md rounded-3xl border border-slate-800 bg-slate-900 p-6">
      ...
      <p className="mt-1 text-sm text-slate-400">Access your account and continue shopping.</p>
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <button className="w-full rounded bg-emerald-500 px-4 py-2 font-semibold text-slate-950" disabled={loading}>{loading ? 'Signing in...' : 'Login'}</button>
      </form>
    </div>
  )
}
